...
class Node:
    BASE_PIPES = {
        ...
        '+': [True, True, True, True],
        'E': [True, False, False, False] # Ống cụt 1 đầu (IN/OUT)
    }
...
class Board:
    def __init__(self, level_id=1):
        ...
        # 1. THIẾT LẬP KÍCH THƯỚC BẢN ĐỒ THEO LEVEL
        if self.level_id == 1:
            self.rows, self.cols = 3, 3
        elif self.level_id == 2:
            self.rows, self.cols = 5, 5
        elif self.level_id in [3, 4]:
            self.rows, self.cols = 7, 7
        else: # Từ level 5 trở đi
    ...
    def generate_board(self):
        ...
        # BẢO VỆ TUYỆT ĐỐI ĐIỂM IN VÀ OUT
        req_conns[0][0][1] = True
        req_conns[0][1][3] = True
        req_conns[self.rows-1][self.cols-1][3] = True
        req_conns[self.rows-1][self.cols-2][1] = True
    ...
    def draw(self, screen):
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                # --- SỬ DỤNG GFXDRAW ĐỂ KHỬ RĂNG CƯA ---
                ...
                # VẼ VAN CHỐT CỐ ĐỊNH TẠI (0,0) VÀ GÓC DƯỚI
                if row == 0 and col == 0:
